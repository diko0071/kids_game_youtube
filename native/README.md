...
- Main-frame navigation permits only HTTPS on the existing app host and catalog video IDs and up to two English catalog entries recommended by its active embed. YouTube frame navigation permits those playback IDs. Recommendation ranking expires when another video starts. The macOS Debug host can use an explicitly configured loopback preview URL. New windows, watch pages, channels, external links, and custom URL schemes are rejected.
- The native bridge reads up to 20 real recommendation cards from the active YouTube embed, validates their source and fields, then takes the first two matching the English catalog in YouTube order. Those entries precede the deduplicated app catalog in For You. Unknown videos are omitted even when their title is English; fewer than two matches means fewer than two recommendation cards. Other categories use the curated catalog. No recommendations are fabricated or persisted. The ordinary website cannot read the cross-origin embed and displays the curated catalog only. The catalog contains 23 English editions. Player UI and caption preferences use English; these parameters do not guarantee a particular audio track if YouTube changes its dubbing defaults.
