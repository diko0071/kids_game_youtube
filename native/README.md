...
The iOS app opens the existing catalog in WKWebView and injects `shared/player-policy.js` into YouTube embed frames at document start. It hides recommendation entry points and end-screen cards inside those frames. The mobile catalog offers Feed, Carousel, and Grid layouts under parent settings. In phone landscape (and touch-device landscape), the player fills the app viewport, with no header or side catalog. A pause button overlays the video; pausing opens the full catalog. Rotation preserves the same player instance. The iOS host extends into safe areas and hides system overlays in landscape. Learning interruptions are off by default, including when migrating saved settings. A macOS build of the same Swift/WebKit code supports local verification without starting an iOS simulator.
...
- Main-frame navigation permits only HTTPS on either of the two production app hosts (`mira-luke.vercel.app` and `kids.dkravt.ai`) and catalog video IDs plus the current video and up to two recommendations from its active embed. YouTube frame navigation permits those playback IDs. Recommendation grants expire when another video starts; the selected video remains playable. The macOS Debug host can use an explicitly configured loopback preview URL; a preview origin is isolated to itself and does not inherit the production hosts. New windows, watch pages, channels, external links, and custom URL schemes are rejected inside the WebView. The parent-settings search is the sole external exception: a top-level app request to https://www.youtube.com/results with one nonempty search_query (at most 200 characters) opens the system browser or YouTube app. YouTube iframe requests cannot use this exception.
- The native bridge reads the first two real recommendation cards from the active YouTube embed and validates their source and fields. Those entries precede the deduplicated app catalog in For You, regardless of language or catalog membership. Unknown language stays unknown, rather than inheriting the current video language. If YouTube supplies fewer than two cards, no substitutes are invented. Other categories use the curated catalog. No recommendations are fabricated or persisted. The ordinary website cannot read the cross-origin embed and displays the curated catalog only. The bundled catalog contains 31 English editions, including 10 vehicle videos. Player UI and caption preferences use English; these parameters do not guarantee a particular audio track if YouTube changes its dubbing defaults.
