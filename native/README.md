...
- Main-frame navigation permits only HTTPS on the existing app host and catalog video IDs plus the current video and up to two recommendations from its active embed. YouTube frame navigation permits those playback IDs. Recommendation grants expire when another video starts; the selected video remains playable. The macOS Debug host can use an explicitly configured loopback preview URL. New windows, watch pages, channels, external links, and custom URL schemes are rejected inside the WebView. The parent-settings search is the sole external exception: a top-level app request to https://www.youtube.com/results with one nonempty search_query (at most 200 characters) opens the system browser or YouTube app. YouTube iframe requests cannot use this exception.
