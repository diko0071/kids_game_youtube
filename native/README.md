...
- The native bridge reads the first two real recommendation cards from the active YouTube embed and validates their source and fields. Those entries precede the deduplicated app catalog in For You, regardless of language or catalog membership. Unknown language stays unknown, rather than inheriting the current video language. If YouTube supplies fewer than two cards, no substitutes are invented. Other categories use the curated catalog. No recommendations are fabricated or persisted. The ordinary website cannot read the cross-origin embed and displays the curated catalog only. The catalog contains 23 English editions. Player UI and caption preferences use English; these parameters do not guarantee a particular audio track if YouTube changes its dubbing defaults.
